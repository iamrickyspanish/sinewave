...
void list_unshift (list l, void* item) {
    ...
    for (unsigned short i = 0; i < l->length - 1; i++) {
        memcpy (l->items + (i + 1) * l->item_size, l->items + i * l->item_size,
        l->item_size);
    }
...
void list_shift (list l, void* buffer) {
    ...
    for (unsigned short i = 0; i < l->length - 1; i++) {
        memcpy (l->items + i * l->item_size, l->items + (i + 1) * l->item_size,
        l->item_size);
    }
    l->length--;
};
    ...
    for (unsigned short i = 0; i < l->length; i++) {
        if (!(*fn) (l->items + i * l->item_size, i, data)) {
            for (unsigned short j = i; j < l->length - 1; j++) {
                memcpy (l->items + j * l->item_size,
                l->items + (j + 1) * l->item_size, l->item_size);
            }
            l->length--;
        };
    }
...
list_item_to_string_fn item_to_string_fn,
unsigned short item_string_length) {
    char items_as_string[l->length][item_string_length];
    strcpy (string, "");
    if (l->length == 0) {
        return;
    }
    unsigned short relevant_item_count =
    (item_string_length == 0) ? 0 : string_length / item_string_length;
    if (relevant_item_count == 0) {
        return;
    }
    for (unsigned short i = 0; i < relevant_item_count; i++) {
        (*item_to_string_fn) (l->items + i * l->item_size, items_as_string[i],
        string_length, item_string_length);
